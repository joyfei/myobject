...
urlpatterns = [
    ...
    path('verify', index.verify, name="myadmin_verify"),#验证码输出

    # 员工信息管理路由
    path('user/<int:pIndex>', user.index, name="myadmin_user_index"),#浏览
    path('user/add', user.add, name="myadmin_user_add"),#添加表单
    path('user/insert', user.insert, name="myadmin_user_insert"),#执行表单
    ...
    path('user/update/<int:uid>', user.upedit, name="myadmin_user_upedit"),#执行表单
